...
function validateStrings(stringA, stringB, iterators){
    ...
    if(charA === charB){
        ...
        var charAHex = hexEncodeChar(stringA, iterators.A);
        if(charsToOmit.indexOf(charAHex) >= 0){
            iterators.out += charA;
            iterators.A++;
            result = true;
        }
        var charBHex = hexEncodeChar(stringB, iterators.B);
    ...
    if(!result){
        console.log(iterators, stringA, stringA.substr(iterators.A, 10), stringB.substr(iterators.B, 10));
        process.exit();
    }
    return result;
}
        ...
        .then(function (htmlData) {
            ...
            return new Promise(function(resolve, reject){
                        ...
                        var parser = new htmlparser.Parser({
                            ...
                            ontext: function(text){
                                ...
                                var evaluatePageChange = function(){
                                    if(pages[page] && pages[page].text.length === iterators.B){
                                        insertPageNumber = true;
                                        page++;
                                        iterators.B = 0;
                                        //console.log('---------------------------->Page', page);
                                        iterators.out += `<?textpage page-num="${page}" release-num="Août 2017"?>`;
                                    }
                                }
                                ...
                                if(onBody && text.length > 0){
                                    ...
                                    iterators.A = 0;
                                    while(pages[page] && iterators.A < text.length && iterators.B < pages[page].text.length){
                                        if(!validateStrings(text, pages[page].text, iterators)){
                                            break;
                                        }
                                        evaluatePageChange();
                                    }
                            ...
                            onend: function(){
                                ...
                                fsWriteFile(htmlOutPath + '/' + fileName + '.extracted.html', iterators.out, 'utf8')
                                .then(() => {
                                    resolve(true);
                                })
                            }
                ...
                var pages = {},
                    ...
                    it = 0,
                    lastP,
                    footerRegExp = /^\([0-9]+\)[0-9]+\s\/\s[0-9]+[a-zA-Z]+\s*[0-9]*/,
                    leftHeaderExp = /I\. Aspects généraux/,
                    rightHeaderExp = /^\s*Fasc\.\s[0-9]/;
                ...
                var parser = new htmlparser.Parser({
                    onopentag: function(name, attribs){
                        ...
                        if(name === "page"){
                            ...
                            if(found){
                                lastP = found[1];
                                pages[lastP] = {text: '', footers: [], headers: {}};
                            }
                            it++;
                        }
                    },
                    ontext: function(text){
                        if(lastNodeName === "content"){
                                
                            if( footerRegExp.test(text) ){
                                pages[lastP].footers.push(text);
                                lastText = '';
                            
                            }else if( leftHeaderExp.test(text) ){
                                pages[lastP].headers.left = text;
                                lastText = '';

                            }else if( rightHeaderExp.test(text) ){
                                pages[lastP].headers.right = text;
                                lastText = '';
                    ...
                    onend: function(){
                        var inlineHtmlFile = "6018_JCQ_10-F03_MJ9.indd.inline.html";
